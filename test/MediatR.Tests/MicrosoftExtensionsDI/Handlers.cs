...
namespace MediatR.Tests.MicrosoftExtensionsDI
{
    ...
    public class DerivedPingHandler(Logger logger) : IRequestHandler<DerivedPing, Pong>
    {
    ...
    public class ZingHandler(Logger output) : IRequestHandler<Zing, Zong>
    {
    ...
    public class PingStreamHandler(Logger output) : IStreamRequestHandler<StreamPing, Pong>
    {
    ...
    public class DuplicateHandler1 : IRequestHandler<DuplicateTest, string>
    {
        public Task<string> Handle(DuplicateTest message, CancellationToken cancellationToken)
            => Task.FromResult(nameof(DuplicateHandler1));
    }
...
namespace MediatR.Tests.MicrosoftExtensionsDI.Included
{
    ...
    public class FooHandler(Logger logger) : IRequestHandler<Foo, Bar>
    {
